...
prop_tab <- function(v, decimales) {
  ...
  tri <- round(tri / effectif_total * 100, decimales) # Il faut modifier la fonction round, en lui donnant l'argument decimales.
  return(tri)
}

# Tapez submit() une fois que vous avez fini de modifier le script.